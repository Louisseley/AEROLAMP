   ...
   const submission = async (data) => {
      ...
      .then((response) => {
         ...
         if (response.data.token) {
             ...
             console.log('Token saved:', response.data.token);  // Log the token to ensure it's being saved
             navigate(`/home`); // Navigate to the home page or wherever you need
         } else {
         ...
         <div className="bg-cover bg-center bg-no-repeat h-screen flex items-center justify-center" style={{ backgroundImage: "url('/images/bg1.jpg')" }}>
            <div className=" w-[40%] h-[80%] flex flex-col items-center">
               ...
               <form onSubmit={handleSubmit(submission)} className="w-[100%] h-[40%] flex flex-col items-center justify-center">
                  ...
                  <a className="font-inknut font-normal text-[18px] ml-[40%] no-underline hover:underline mt-[2%] link text" href="/forgotpassword">Forgot Password</a>
                  <button type={"submit"} className="font-inknut font-normal text-[20px] px-[10px] py-[5px] round mt-[5%] cursor-pointer button-login hover:opacity-40">Log  In </button>
               </form>
               <p className="font-inknut font-normal text-[18px] color mt-[1%] text w-[100%] text-center">No account yet? <span className="span link cursor-pointer" onClick={() => {navigate('/register')}}>Sign Up</span></p>
            </div>
         </div>
