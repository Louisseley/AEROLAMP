...
const Aerolamp = () => {
   ...
   const registerDevice = async () => {
      ...
      try {
         const response = await AxiosInstance.post("devices/register_device/", {
   ...
   return (
      <div className="bg1 flex items-center justify-center relative w-[100%] h-[100%]">
         ...
         <div className="grid grid-cols-3 grid-rows-2 w-[70%] h-[85%] gap-[30px] place-items-center mt-[80px] cursor-pointer aerolamp-device">
            {
               devices.map((device) => (
                  <Link key={device.id} to={`/aerolamp/${device.id}`} className="h-[100%] w-[85%] bg4 shadow-md flex flex-col items-center font-inknut font-normal color pt-[5%] aerolamp">
            ...
            <div className="absolute w-[100%] h-[100%] z-10 top-0 bottom-0 left-0 right-0 bg-[#00000080] flex items-center justify-center font-inknut font-normal text-[20px]">
               <div className="w-[30%] h-[70%] flex flex-col items-center justify-center bg5 relative">
                  ...
                  <button
                     ...
                     className=" text-white px-4 py-2 rounded h-[30px] w-[40%] bg-[#24549d] color mt-[20px]"
                  >
                     Connect ESP32
                  </button>
               </div>
